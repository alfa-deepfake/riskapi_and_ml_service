...
поэтому risk-api там опубликован на **8002** через `docker-compose.override.yml`
(см. раздел «Порт занят»).
...
Найти свободный порт и переопределить публикацию через override-файл (compose
подхватывает `docker-compose.override.yml` автоматически; тег `!override`
**заменяет** маппинг, а не добавляет второй):
```bash
cat > docker-compose.override.yml <<'YAML'
services:
  risk-api:
    ports: !override
      - "8002:8000"          # 8002 — любой свободный порт хоста
YAML

docker compose up -d
```
После этого risk-api снаружи доступен на `http://localhost:8002`, а ml внутри
по-прежнему ходит на `risk-api:8000`. На кластере этот override уже создан.

Аналогично можно сдвинуть `frontend`/`mongo`. **`ml-service` (8100) двигать
нежелательно** — на него завязан `frontend/config.js`; если двигаете, поправьте
там `mlApiUrl`.
